...
RVP_DFF\n\
	...
	float getWenergy() {
	   ...
	   cout << "delay_DFF_w= " << delay_DFF_w << "\n energy_DFF_r_w= " << energy_DFF_r_w << endl;
	}
};
...
RVP_SA\n\
        ...
        void extractOutput() {
                ...
                if(!fileHandle.is_open()) {
			cerr << "Error: Can't open output file \n ../results/DFF/RVP_SA/datar.txt" << endl;
			exit(1);
		}
		sst << fileHandle.rdbuf();
	...
	void extractOutput() {
		...
		sst.str("");
	        ...
	        // Pass-gate leakage
                ...
                if(!fileHandle.is_open()) {
			cerr << "Error: Can't open output file \n ../results/BC/RVP_Ileak_PG/data.txt" << endl;
			exit(1);
		}
                sst << fileHandle.rdbuf();
                sst >> iOffPg; 
		...
		sst.str("");
                ...
                if(!fileHandle.is_open()) {
			cerr << "Error: Can't open output file \n ../results/BC/RVP_Ileak_PU/data.txt" << endl;
			exit(1);
		}
                sst << fileHandle.rdbuf();
                sst >> iOffPu; 
		...
		sst.str("");
                ...
                if(!fileHandle.is_open()) {
			cerr << "Error: Can't open output file \n ../results/BC/RVP_Ileak_PD/data.txt" << endl;
			exit(1);
		}
                sst << fileHandle.rdbuf();
                sst >> iOffPd; 
