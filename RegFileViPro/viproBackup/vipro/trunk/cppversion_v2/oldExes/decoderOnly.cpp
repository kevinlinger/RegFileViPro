...
/*
			...
			cout << "dynamic write delay = " << DR.getDynamicWriteDelay() << " dynamic write energy = " << DR.getDynamicWriteEnergy() << endl;
		}
...
class calculator {
	...
	void sweep() {
		...
		ofile << inputHandle.sweepToken << ",DynamicReadDelay,DynamicReadEnergy,StaticReadDelay,StaticReadEnergy,DynamicWriteDelay,DynamicWriteEnergy,StaticWriteDelay,StaticWriteEnergy,DecoderDelay,DecoderEnergy\n";
		...
		for (float var = inputHandle.sweepBegin; var <= inputHandle.sweepEnd; var+=inputHandle.sweepStep) {
			...
			ofile << var << "," << sram->DR.getDynamicReadDelay() << "," << sram->DR.getDynamicReadEnergy() << "," << sram->DR.getStaticReadDelay() << "," << sram->DR.getStaticReadEnergy() << "," 
				<< sram->DR.getDynamicWriteDelay() << "," << 
				sram->DR.getDynamicWriteEnergy() << "," << sram->DR.getStaticWriteDelay() << "," << sram->DR.getStaticWriteEnergy() << "," << sram->RD.getDelay() << "," << sram->RD.getEnergy() << endl;
