...
void userInput::print() {
	cout <<  "\nPRINTING USER INPUT INFO" << "\n"
		 ...
		 for(int i = 0; i < knobCount; ++i) {
		 	cout << knobName[i] << " " << knobMin[i] << " " << knobMax[i] << endl;
		 }
}
...
int userInput::getWritePorts() {
	return writePorts;
}

void userInput::setWritePorts(int numWritePorts) {
	writePorts = numWritePorts;
}

void userInput::setBitcellUpsize(int size) {
	bitcellUpsize = size;
}

int userInput::getBitcellUpsize() {
	return bitcellUpsize;
}

int userInput::getReadPorts() {
	return readPorts;
}

void userInput::setReadPorts(int numReadPorts) {
	readPorts = numReadPorts;
}
...
class parser {
	...
	public:
		...
		int parseUserToken(string& str) {
			...
			} else if (token == "ReadPorts" ) {
				input_handle.setReadPorts(ivalue);
			} else if (token == "WritePorts") {
				input_handle.setWritePorts(ivalue);
			} else if (token == "BitcellUpsize") {
				input_handle.setWritePorts(ivalue);
			} else {
				cerr << "Error: Can't Parse Token at \n" << str << endl;
				exit(1);
...
RVP_Gate_Capacitance\n\
		...
		void constructTemplate() {
			...
			while(!tpl.eof()) {
				...
				} else if(line.find("<pvbp>") != string::npos) {
					techTemplate << "<pvbp> " << inp.vdd << endl;
...
<char>\t0\n"; 		
			...
			string sizePath = "../configuration/kevin/bitcellSizes_" + tech + ".m";
			cout << sizePath << endl;
			ifstream bc(sizePath.c_str());
			techTemplate << "\
#############################\n\
...
#############################\n";
		...
		void extractOutput() {
			...
			DR.extractOutput();
			cout << "RegFile Output extracted" << endl;
...
*/
			read_delay = DR.getDynamicReadDelay();
			read_energy = DR.getDynamicReadEnergy();
			cout << "dynamic read delay = " << DR.getDynamicReadDelay() << " dynamic read energy = " << DR.getDynamicReadEnergy() << endl;
			cout << "static read delay = " << DR.getStaticReadDelay() << " static read energy = " << DR.getStaticReadEnergy() << endl;
			cout << "Row decoder delay: " << RD.getDelay() << " decoder energy: " << RD.getEnergy()*(inp.getReadPorts()+inp.getWritePorts()) << endl;
...
/*
			...
			write_delay = DR.getDynamicWriteDelay();
			write_energy = DR.getDynamicWriteEnergy();
			cout << "dynamic write delay = " << DR.getDynamicWriteDelay() << " dynamic charge delay = " << DR.getDynamicWriteChargeDelay() << " dynamic write energy = " << DR.getDynamicWriteEnergy() << endl;
			cout << "static write delay = " << DR.getStaticWriteDelay() << " static charge delay = " << DR.getStaticWriteChargeDelay() <<" static write energy = " << DR.getStaticWriteEnergy() << endl;
		}
...
class calculator {
	...
	void sweep() {
		...
		ofile << inputHandle.sweepToken << ",DynamicReadDelay,DynamicReadEnergy,StaticReadDelay,StaticReadEnergy,DynamicWriteDelay,DynamicWriteChargeDelay,DynamicWriteEnergy,StaticWriteDelay,StaticWriteChargeDelay,StaticWriteEnergy,DecoderDelay,DecoderEnergy\n";
		...
		for (float var = inputHandle.sweepBegin; var <= inputHandle.sweepEnd; var+=inputHandle.sweepStep) {
			...
			else if(!strcmp(inputHandle.sweepToken.c_str(), "VDD")) {
				inputHandle.vdd = (float)var;
			}
			else if(!strcmp(inputHandle.sweepToken.c_str(), "ReadPorts")) {
				inputHandle.readPorts = (int)var;
			}
			else if(!strcmp(inputHandle.sweepToken.c_str(), "WritePorts")) {
				inputHandle.writePorts = (int)var;
			}
			else if(!strcmp(inputHandle.sweepToken.c_str(), "BitcellUpsize")) {
				inputHandle.bitcellUpsize = (int)var;
			}
			sram->setInput(inputHandle);
			...
			//Append CSV file
			//",DynamicReadDelay,DynamicReadEnergy,StaticReadDelay,StaticReadEnergy,DynamicWriteDelay,DynamicWriteEnergy,StaticWriteDelay,StaticWriteEnergy,DecoderDelay,DecoderEnergy\n";
			//ofile << var << "," << rD << "," << rE << "," << wE << "," << wD << endl;
			ofile << var << "," << sram->DR.getDynamicReadDelay() << "," << sram->DR.getDynamicReadEnergy() << "," << sram->DR.getStaticReadDelay() << "," << sram->DR.getStaticReadEnergy() << "," 
				<< sram->DR.getDynamicWriteDelay() << "," << sram->DR.getDynamicWriteChargeDelay() << "," <<
				sram->DR.getDynamicWriteEnergy() << ","  << sram->DR.getStaticWriteDelay() << "," << sram->DR.getStaticWriteChargeDelay() << "," << sram->DR.getStaticWriteEnergy() << "," << 
				sram->RD.getDelay() << "," << sram->RD.getEnergy()*((inputHandle.getReadPorts()+inputHandle.getWritePorts())) << endl;
		
			
			//write_delay = DR.getDynamicWriteDelay();
			//write_energy = DR.getDynamicWriteEnergy();
			//cout << "dynamic read delay = " << DR.getDynamicReadDelay() << " dynamic read energy = " << DR.getDynamicReadEnergy() << endl;
			//cout << "Row decoder delay: " << RD.getDelay() << " decoder energy: " << RD.getEnergy() << endl;
