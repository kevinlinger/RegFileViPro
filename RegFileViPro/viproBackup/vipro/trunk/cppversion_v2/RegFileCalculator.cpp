...
void userInput::print() {
	cout <<  "\nPRINTING USER INPUT INFO" << "\n"
		 ...
		 "calNumColMux: " << calNumColMux << "\n"
		 "readPorts: " << readPorts << "\n"
		 "writePorts: " << writePorts << endl;
	...
	public:
		...
		void charGateCap() {
			...
			if (tpl.is_open()) {
				...
				cerr << "Error3: Can't find input template.\n";
				exit(1);
			}
			new_tpl << "\n\
<ocn>\n\
RVP_Gate_Capacitance\n\
		...
		void constructTemplate() {
			...
			//templ_path << tasePath << "/template/RVPtpl_" << tech << ".ini";
			templ_path << tasePath << "/template/RVPtpl_" << "ibm45" << ".ini";
...
#############################\n";
		...
		//	float energy_timing = TB.getEnergy();
			...
			float dynamicReadDelay = DR.getDynamicReadDelay();
			float dynamicReadEnergy = DR.getDynamicReadEnergy();
		...
		//	read_energy=leakage_power*read_delay+energy_timing+energy_DFF_r+energy_rowDecoder+energy_pch_r+energy_bitcell_r+energy_bankMux+energy_SA+energy_sa_inter+(2*energy_bm_inter)+energy_inter_bc_read;
			...
			read_delay = delay_DFF+max(delay_pch_r+delay_inter_bc_read,delay_rowDecoder)+dynamicReadDelay;
			read_energy = leakage_power*read_delay+energy_DFF_r+energy_rowDecoder+energy_pch_r+energy_bitcell_r+energy_inter_bc_read+dynamicReadEnergy;
		...
		void calculateWriteED(float& write_energy, float& write_delay) {
			...
			// Bank Mux not needed for regFile 
			//float delay_bankMux = BM.getBankMuxtDelay();
			//float delay_bm_inter = BM.getIntDelay();
			//float energy_bankMux = BM.getBankMuxtEnergy();
			//float energy_bm_inter = BM.getIntEnergy();
			...
			write_delay = delay_DFF+max(max(delay_inter_bc_read+delay_pch_w,delay_rowDecoder),(max(delay_DFF_w-delay_DFF,delay_inter_bc_write)+delay_writeDriver))+delay_bitcell_w;
			write_energy = leakage_power*write_delay+energy_DFF_w+energy_rowDecoder+energy_writeDriver+energy_pch_w+energy_bitcell_w+energy_inter_bc_write+energy_inter_bc_read;
