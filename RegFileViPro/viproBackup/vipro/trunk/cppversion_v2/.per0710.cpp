...
class parser {
	...
	public:
		...
		int parseSWEEPToken(string& str) {
			...
			int status = regexec(&re, str.c_str(), 1, pmatch, 0); 
			regfree(&re); 
			if(!status) {
				cerr << "Error parsing VIPRO::END" << endl;
				exit(1);
			}
			str.erase(pmatch[0].rm_so, pmatch[0].rm_eo - pmatch[0].rm_so);
			return 0;
		...
		string checkCmd(string str) {
			if(sscanf(str.c_str(),"SWEEP")) {
				return "SWEEP";
			} 
			else if(sscanf(str.c_str(),"OPTIMIZE")) {
				return "OPTIMIZE";
			} 
		}
...
class calculator {
	...
	void sweep() {
		...
		float rE, rD, wE, wD;
		ofstream ofile("../results/sweep.txt");
		for (float offset = 0.05; offset < 0.3; offset+=0.05) {
			
			// Modify the input 
			inputHandle.SAoffset = offset;
			sram->setInput(inputHandle);
