...
if exist('RSTSRAM_WM_WL/OUT/DAT0','dir')   
  %load data%
  f0=load('RSTSRAM_WM_WL/OUT/DAT0/monteCarlo/mcdata');
  %get WriteMargin%
  WM0=f0(:,1)*1e3; % in mV
  QBmQ=f0(:,2);
  ...
  fout1 = fopen('RSTSRAM_WM_WL/OUT/wl_d0.out','w');
  fprintf(fout1, '%f\n', WM0);
  fclose(fout1);
  %get mean and std
  [m,s]=normfit(WM0);
  ...
  %save and close%
  print(gcf, '-depsc', 'RSTSRAM_WM_WL/IMG/SRAM_WM0_WL.eps');
  close(gcf);
end

%%%%%%%%% For Write '0' into the cell that holds '1' %%%%%%%%%%%
if exist('RSTSRAM_WM_WL/OUT/DAT1','dir')   
  %load data%
  f1=load('RSTSRAM_WM_WL/OUT/DAT1/monteCarlo/mcdata');
  %get WriteMargin%
  WM1=f1(:,1)*1e3; % in mV
  QBmQ=f1(:,3);
  %replace nil value '-1.11111e+36' to 0%
  ...
  %save write margin data in a file%
  fout1 = fopen('RSTSRAM_WM_WL/OUT/wl_d1.out','w');
  fprintf(fout1, '%f\n', WM1);
  fclose(fout1);
  %get mean and std
  [m,s]=normfit(WM1);
  ...
  %save and close%
  print(gcf, '-depsc', 'RSTSRAM_WM_WL/IMG/SRAM_WM1_WL.eps');
  close(gcf);
end
...
if (size(f0,1)>0 & size(f1,1)>0)
  ...
  %save and close%
  print(gcf, '-depsc', 'RSTSRAM_WM_WL/IMG/RSTSRAM_WM_WL.eps');
  close(gcf);
end
