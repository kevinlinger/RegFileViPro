...
if exist('5TSRAM_RVCCmin/OUT/DAT0','dir')   
  %load data%
  f0=load('5TSRAM_RVCCmin/OUT/DAT0/monteCarlo/mcdata');
  %get WriteMargin%
  ...
  fout1 = fopen('5TSRAM_RVCCmin/OUT/rvcc_d0.out','w');
  fprintf(fout1, '%f\n', f0(:,1));
  fclose(fout1);
  %get mean and std
  [m,s]=normfit(VCCmin0);
  ...
  %save and close%
  print(gcf, '-depsc', '5TSRAM_RVCCmin/IMG/5TSRAM_RVCCmin0.eps');
  close(gcf);
end

%%%%%%%%% For Write '0' into the cell that holds '1' %%%%%%%%%%%
if exist('5TSRAM_RVCCmin/OUT/DAT1','dir')   
  %load data%
  f1=load('5TSRAM_RVCCmin/OUT/DAT1/monteCarlo/mcdata');
  %get WriteMargin%
  ...
  VCCmin1(id)=0;
  %save write margin data in a file%
  fout1 = fopen('5TSRAM_RVCCmin/OUT/rvcc_d1.out','w');
  fprintf(fout1, '%f\n', f1(:,1));
  fclose(fout1);
  %get mean and std
  [m,s]=normfit(VCCmin1);
  ...
  %save and close%
  print(gcf, '-depsc', '5TSRAM_RVCCmin/IMG/5TSRAM_RVCCmin1.eps');
  close(gcf);
end
...
if (size(f0,1)>0 & size(f1,1)>0)
  ...
  %save and close%
  print(gcf, '-depsc', '5TSRAM_RVCCmin/IMG/5TSRAM_RVCCmin.eps');
  close(gcf);
end
