...
if exist('SRAM_RVCCmin/OUT/DAT0','dir')   
  %load data%
  f0=load('SRAM_RVCCmin/OUT/DAT0/monteCarlo/mcdata');
  %get VCCmin%
  ...
  fout1 = fopen('SRAM_RVCCmin/OUT/rvcc_d0.out','w');
  fprintf(fout1, '%f\n', f0(:,1));
  fclose(fout1);
  %get mean and std
  [m,s]=normfit(VCCmin0);
  ...
  %save and close%
  print(gcf, '-depsc', 'SRAM_RVCCmin/IMG/SRAM_RVCCmin0.eps');
  close(gcf);
end

%%%%%%%%% For Write '0' into the cell that holds '1' %%%%%%%%%%%
if exist('SRAM_RVCCmin/OUT/DAT1','dir')   
  %load data%
  f1=load('SRAM_RVCCmin/OUT/DAT1/monteCarlo/mcdata');
  %get VCCmin%
  ...
  VCCmin1(id)=0;
  %save VCCmin data in a file%
  fout1 = fopen('SRAM_RVCCmin/OUT/rvcc_d1.out','w');
  fprintf(fout1, '%f\n', f1(:,1));
  fclose(fout1);
  %get mean and std
  [m,s]=normfit(VCCmin1);
  ...
  %save and close%
  print(gcf, '-depsc', 'SRAM_RVCCmin/IMG/SRAM_RVCCmin1.eps');
  close(gcf);
end
...
if (size(f0,1)>0 & size(f1,1)>0)
  ...
  %save and close%
  print(gcf, '-depsc', 'SRAM_RVCCmin/IMG/SRAM_RVCCmin.eps');
  close(gcf);
end
