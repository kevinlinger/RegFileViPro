...
dc dc                           \
	...
	stop=pvdd		\
	step=0.001
export HSNMh=oceanEval("ymax(clip(v(\"v1mv2h\" ?result 'dc) nil 0))/sqrt(2)*1e3")
export HSNMl=oceanEval("ymin(clip(v(\"v1mv2h\" ?result 'dc) 0 nil))/sqrt(2)*(-1)*1e3")
export HSNM=oceanEval("min(ymax(clip(v(\"v1mv2h\" ?result 'dc) ymax(v(\"VDD\" \
