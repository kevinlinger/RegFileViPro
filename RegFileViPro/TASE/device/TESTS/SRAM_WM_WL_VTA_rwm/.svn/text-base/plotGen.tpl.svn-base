...
if exist('SRAM_WM_WL_VTA_rwm/OUT/DAT0','dir')   
  %load data%
  f0=load('SRAM_WM_WL_VTA_rwm/OUT/DAT0/monteCarlo/mcdata');
  %get WriteMargin%
  WM0=f0(:,1)*1e3; % in mV
  WM0min=min(f0(:,1))*1e3;
  QBmQ=f0(:,2);
  ...
  fout1 = fopen('SRAM_WM_WL_VTA_rwm/OUT/wl_d0.out','w');
  fprintf(fout1, '%f\n', WM0);
  fclose(fout1);
  %get mean and std
  [m0,s0]=normfit(WM0);
 ...
 %fout0 = fopen('SRAM_WM_WL_VTA_rwm/data.txt','w');
 % fprintf(fout0, '%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n', [m0 s0 wmwcf VDD VWL VBL node temp]);
 % fclose (fout0);

  %save and close%
  print(gcf, '-depsc', 'SRAM_WM_WL_VTA_rwm/IMG/SRAM_WM0_WL.eps');
  close(gcf);
end

%%%%%%%%% For Write '0' into the cell that holds '1' %%%%%%%%%%%
if exist('SRAM_WM_WL_VTA_rwm/OUT/DAT1','dir')   
  %load data%
  f1=load('SRAM_WM_WL_VTA_rwm/OUT/DAT1/monteCarlo/mcdata');
  %get WriteMargin%
  WM1=f1(:,1)*1e3; % in mV
  QmQB=f1(:,2);
  WM1min=min(f0(:,1))*1e3;
  ...
  %save write margin data in a file%
  fout1 = fopen('SRAM_WM_WL_VTA_rwm/OUT/wl_d1.out','w');
  fprintf(fout1, '%f\n', WM1);
  fclose(fout1);
  %get mean and std
  [m1,s1]=normfit(WM1);
...
d1=load('SRAM_WM_WL_VTA_rwm/OUT/wl_d1.out');
d0=load('SRAM_WM_WL_VTA_rwm/OUT/wl_d0.out');
wmd0=(d0(:,1));
wmd1=(d1(:,1));
wmf=min(wmd0,wmd1);
wmd0wc=min(d0(:,1));
...
wmwcf=min(wmd0wc,wmd1wc);
  ...
  fout = fopen('SRAM_WM_WL_VTA_rwm/msdata.txt','w');
  fprintf(fout, '%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n', [m s wmwcf VDD VWL VBL node temp m0 s0 LBL VSS pwell nwell VDDa pdvtar pdvtal pgvtar pgvtal puvtar puvtal m1 s1 ]);
  fclose (fout);

%  fout = fopen('SRAM_WM_WL_VTA_rwm/data.txt','w');
%  fprintf(fout2, '%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n', [VDD VWL VBL node m s wmwcf m0 s0 LBL]);    
%  fclose (fout2);
  
  %save and close%
  print(gcf, '-depsc', 'SRAM_WM_WL_VTA_rwm/IMG/SRAM_WM_WL_rwm.eps');
 % print(gcf, '-dtiff','-r300','SRAM_WM_WL_VTA_rwm/IMG/SRAM_WM_WL_rwm.tif');
  close(gcf);
end
