...
%load data%
load RVP_Ion_P/DAT/dc_dc.dat;
load RVP_Ion_P/DAT/dc_P1_PX_d.dat;
load RVP_Ion_P/DAT/dc_P2_PX_d.dat;
load RVP_Ion_P/DAT/dc_P3_PX_d.dat;
load RVP_Ion_P/DAT/dc_P4_PX_d.dat;
load RVP_Ion_P/DAT/dc_P5_PX_d.dat;
load RVP_Ion_P/DAT/dc_P5_PX_g.dat;
...
[mindiff1, idelV] = min(abs(abs(dc_dc) - <deltaVBL>));
[mindiff2, idelVby2] = min(abs(abs(dc_dc) - <deltaVBL>/2));
I3 = abs(I(idelV,5)); I4 = abs(I(idelVby2,5));

%get Ig in unit pA/um2%
%Ig=abs(dc_P5_PX_g(1))*1e12/(W*1e6)/(Leff*1e6);
%save the Ion/Ig values into a file
%fout = fopen('RVP_Ion_P/data.txt', 'w');
%fprintf(fout, '%e\t%e\n', [Ion Ig]'); 
%fclose(fout);
...
%save and close%
print(gcf, '-depsc', 'RVP_Ion_P/IMG/RVP_Ion_P.eps');
close(gcf);
