...
%load data%
f=load('SRAM_RSNM_vs_VDD/out.dat');
...
plot(VDD,[RSNMh RSNMl],'-o','lineWidth',2);
grid on;
...
fout = fopen('SRAM_RSNM_vs_VDD/data.txt', 'w');
fprintf(fout, '%e\t%e\n', [RSNM_nom k]'); 
fclose(fout);

%save and close%
print(gcf, '-depsc', 'SRAM_RSNM_vs_VDD/IMG/SRAM_RSNM_vs_VDD.eps');
close(gcf);
