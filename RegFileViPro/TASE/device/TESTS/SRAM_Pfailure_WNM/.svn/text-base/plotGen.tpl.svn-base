...
path(path,'./SRAM_Pfailure_WNM');

% load SNM data
snm0=load('SRAM_Pfailure_WNM/OUT/WNM0/monteCarlo/mcdata');
snm1=load('SRAM_Pfailure_WNM/OUT/WNM1/monteCarlo/mcdata');
snm2=load('SRAM_Pfailure_WNM/OUT/WNM2/monteCarlo/mcdata');
snm3=load('SRAM_Pfailure_WNM/OUT/WNM3/monteCarlo/mcdata');
...
mu = 1e3*[mu0 mu1 mu2 mu3];
sig = 1e3*[sig0 sig1 sig2 sig3];
mu0 = 1e3*mu0;
sig0 = 1e3*sig0;
...
title('Predicted Write Vmin','fontsize',16);
...
ylabel('Write Vmin (mV)','fontsize',12);
title('Write Vmin for yield=<yield>','fontsize',16);

% save and close
print(gcf, '-depsc', 'SRAM_Pfailure_WNM/IMG/SRAM_Pfailure_WNM.eps');
close(gcf);
