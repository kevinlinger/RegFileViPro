...
if exist('RSTSRAM_WVCCmin/OUT/DAT0','dir')   
  %load data%
  f0=load('RSTSRAM_WVCCmin/OUT/DAT0/monteCarlo/mcdata');
  %get VCCmin%
  ...
  fout1 = fopen('RSTSRAM_WVCCmin/OUT/rvcc_d0.out','w');
  fprintf(fout1, '%f\n', f0(:,1));
  fclose(fout1);
  %get mean and std
  [m,s]=normfit(VCCmin0);
  ...
  %save and close%
  print(gcf, '-depsc', 'RSTSRAM_WVCCmin/IMG/RSTSRAM_WVCCmin0.eps');
  close(gcf);
end

%%%%%%%%% For Write '0' into the cell that holds '1' %%%%%%%%%%%
if exist('RSTSRAM_WVCCmin/OUT/DAT1','dir')   
  %load data%
  f1=load('RSTSRAM_WVCCmin/OUT/DAT1/monteCarlo/mcdata');
  %get VCCmin%
  ...
  VCCmin1(id)=0;
  %save VCCmin data in a file%
  fout1 = fopen('RSTSRAM_WVCCmin/OUT/rvcc_d1.out','w');
  fprintf(fout1, '%f\n', f1(:,1));
  fclose(fout1);
  %get mean and std
  [m,s]=normfit(VCCmin1);
  ...
  %save and close%
  print(gcf, '-depsc', 'RSTSRAM_WVCCmin/IMG/RSTSRAM_WVCCmin1.eps');
  close(gcf);
end
...
if (size(f0,1)>0 & size(f1,1)>0)
  ...
  %save and close%
  print(gcf, '-depsc', 'RSTSRAM_WVCCmin/IMG/RSTSRAM_WVCCmin.eps');
  close(gcf);
end
