...
%load data%
f=load('SRAM_WM_WL_vs_VDD/out.dat');
...
ylabel('Write Margin via WL Sweep (V)','fontsize',12);
...
fout = fopen('SRAM_WM_WL_vs_VDD/data.txt', 'w');
fprintf(fout, '%e\t%e\n', [WM_nom k]'); 
fclose(fout);

%save and close%
print(gcf, '-depsc', 'SRAM_WM_WL_vs_VDD/IMG/SRAM_WM_WL_vs_VDD.eps');
close(gcf);
