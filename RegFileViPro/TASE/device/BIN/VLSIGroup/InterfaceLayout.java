/*
 *Things to look out for, the paths to all of the files have to be made more generic.
 *We want to make the access to the original .ini files _temp.ini, so that will have to be changed accordingly
 *There is a disparity between the linux versus windows in the running of the batch file, which will have to be accounted for.
 *The Path to the Batch file will have to be changed or added to the program in general.
 *The paths in the Batch file will have to be altered as well... 
 */

/*
 * So at this point, this program should be working perfectly. 
 * It is now some tweaking in the batch file that will be the key, along with adjusting to the path for the file.
 */
...
/*
 * We are going to have to make a differentiation between Linux and Windows Machines. Linux are going to require a Shell Program while Windows
 * will be able to use the Batch Files. As of right now, I have a Batch File, and will make a Shell File.
 */
...
//	final private String pathToTASE = "C:/Users/Student/Documents/VLSIResearch/TASE/TASE/";
	final private String pathToTASEFull = pathToTASE + "device/BIN";
	
//	final private String pathToBatch = "/var/home/vrk7bp/RunCadence.bat";
//	final private String pathToBatch = "C:/Users/Student/Documents/VLSIResearch/RunCadence.bat";
		...
		TaseButton.addActionListener(
				new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						/*
						 * ********************************************************************
						 * Run the Shell Script once the button is pressed.
						 
						
						try {
							Runtime.getRuntime().exec("CadenceShell");
...
/* Here is where the command is run through the CommandPrompt, using the Apache Jar Files...
 * When I run it, I get this error...
 * java.io.IOException: Cannot run program "cadence-mmsim" (in directory "/var/home/vrk7bp/TASE/device/BIN"): java.io.IOException: error=2, No such file or directory
 * It looks like the directory is changed, but for some reason the cadence isn't running. I have the cadence-mmsim file in the BIN Folder, but
 * even when I try to run the program through the command prompt it is "bashing" the command.
 * 
 * I guess that this is another road-block.
* **********************************************************************************************************************************************
*/
						...
						try {
							//This command works for a Linux Machine, the command is different for a Windows Machine
							//Creates a process with the runnings of the Batch File.
							p = Runtime.getRuntime().exec("sh " + pathToBatch);
							
							/*For Windows, the command would be like, 
							 * p = Runtime.getRuntime().exec("cmd start /c" + pathToBatch");
							 */
							/*
							//Obtains and prints out the outcomes of the batch file in the Java Command Prompt below.
						...
						//Shows if any errors come up in the batch file process.
						catch(Exception k) {
...
//							String line;
//					        DefaultExecutor execute = new DefaultExecutor();
//					        CommandLine runCommand = new CommandLine("cadence-mmsim");
//					        runCommand.addArgument("perl run.pl -i " + pathToTASE + "template/" + techDropDownValue + ".ini");
//					        File newDirectory = new File(pathToTASE + "device/BIN");
//					        Process p = execute.launch(runCommand, runCommand.getSubstitutionMap(), newDirectory);
...
//				    	
//				    	CommandLine runCommandTwo = new CommandLine("cadence-mmsim");
//				    	CommandLine runCommandThree = new CommandLine("perl run.pl -i " + pathToTASE + "template/" + techDropDownValue + ".ini");
//				    	DefaultExecutor execute = new DefaultExecutor();
//				        File newDirectory = new File(pathToTASE + "device/BIN");
//				    	execute.setWorkingDirectory(newDirectory);
//				    	ExecuteStreamHandler stream = execute.getStreamHandler();
//				    	stream.start();
////				    	execute.execute(runCommandTwo);
//				    	execute.execute(runCommandThree);
////				        BufferedReader bri = new BufferedReader(new InputStreamReader(p.getInputStream()));
////				        BufferedReader bre = new BufferedReader(new InputStreamReader(p.getErrorStream()));
////				        while ((line = bri.readLine()) != null) {
////				          System.out.println(line);
...
/* This Grouping of Code became iterative and unnecessary...
* 
...
//		 Move file to new directory
		...
		*/
		...
		System.out.println("The Two New Files are Created");
		try {
			InterfaceLayout.copyFile(originalCopy, techCopy);
			System.out.println("The Files are Copied");
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
